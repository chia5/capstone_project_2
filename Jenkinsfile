pipeline {
    agent {label 'Test'}
    options{
        build_Discarder logRotator(artifactDaysToKeepStr: '', artifactNumToKeepStr: '5', daysToKeepStr:'', numToKeepStr: '5')
    ...
    stages{
        ...
        stage('Push To Test Env'){
            agent {label 'Test'}
            steps{
                sh "docker run -d -p 80:80 chash07/capstone_project2:V$BUILD_NUMBER"
